...
public class testImageSphereShading {
    public static double hit_sphere(vec3 center, double radius, ray r) {
        ...
        double halfB = vec3.rDotVec(oc, r.getDir());
    ...
    public static vec3 ray_color(ray r) {
        double t = hit_sphere(new vec3(0.0, 0.0, -1.0), 0.5, r);
        if (t > 0.0) {
            vec3 n = vec3.rUnitVector(vec3.rSubTwoVec(r.at(t), new vec3(0.0, 0.0, -1.0)));
            return vec3.rMultOneVec(new vec3(n.x() + 1, n.y() + 1, n.z() + 1), 0.5);
        }
    ...
    public static void makeImage() {
        ...
        File outFile = join(CWD, "picture5.ppm");
